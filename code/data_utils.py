...
def load_all(train_path, valid_path, test_path, visual_path, category_path):
    ...
    visual_matrix = np.array([visual_features[item] for item in item_ids], dtype=np.float32)  # (num_items, 512)

    # Compute mean and std across all items
    mean_visual = visual_matrix.mean(axis=0, keepdims=True)  # (1, 512)
    std_visual = visual_matrix.std(axis=0, keepdims=True) + 1e-8  # Avoid division by zero
    ...
    for user, items in train_dict.items():
        ...
        for item in items:
            ...
            item_visual_map[int(item)] = normalized_visual_matrix[item_ids.index(item)]
            # Store category ID (unchanged)
            item_category_map[int(item)] = int(category_features.get(int(item), -1))
...
class MFData(data.Dataset):
    def __init__(self, features, num_item, item_visual_map, item_category_map, train_dict=None, is_training=False):
        ...
        print(f"[DEBUG] Initialized MFData: {len(features)} positive interactions loaded")
    ...
    def ng_sample(self):
        ...
        for user, _ in self.features_ps:
            neg_items = set()
            while len(neg_items) < 3:  # Generate 3 negatives per positive sample
        ...
        self.labels_ng = [0 for _ in range(len(self.features_ng))]  # Negative labels (0)
        # Debugging: Print negative samples
        if len(self.features_ng) > 0:
            print(f"[DEBUG] Negative Sampling Example - User {self.features_ng[0][0]}: {self.features_ng[:5]}")

    # Merge
        # Merge positive & negative samples
        self.features_fill = self.features_ps + self.features_ng
        self.labels_fill = self.labels + self.labels_ng
        print(f"[DEBUG] Negative Sampling Done: {len(self.features_ng)} negatives generated")
    ...
    def __getitem__(self, idx):
        ...
        visual_feature = torch.tensor(self.item_visual_map.get(item, np.zeros(512)), dtype=torch.float32)
        category_id = torch.tensor(self.item_category_map.get(item, -1), dtype=torch.long)  # Default to -1 if missing

        if idx < 5:  # Print only first few samples
            print(f"[DEBUG] Data Sample - User: {user}, Item: {item}, Category: {category_id}, Label: {label}")
